<template>
	<div class="flex items-center justify-center h-screen dark:bg-gray-900">
	  <div class="card mx-8 w-full md:w-1/2 lg:w-1/3 bg-base-100 shadow-xl">
		<figure>
			<nuxt-img :src="image" class="w-full h-72 lg:h-96 object-fill"/>
		</figure>
...
<script setup lang="ts">
  let main = "Hämtar skolmat...", alternative = "Var god vänta, detta kan ta en stund.", image = "/img/loading.gif", config = useRuntimeConfig(), { data } = await useFetch(`${config.apiProtocol}://${config.apiHostname}${config.foodEndpoint}`);
